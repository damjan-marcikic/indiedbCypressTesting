export class RegisterPage {
  ...
  registerUserPositive(years, months, days) {
    cy.intercept("POST", "https://www.indiedb.com/members/register/").as(
      "register"
    );
    ...
    cy.get(this.resendEmailConfirmation)
      .should("be.visible")
      .should(
        "contain",
        "An email has been sent with a link to confirm your email address."
      );
  ...
  registerUserNegative(years, months, days, email) {
    ...
    cy.get(this.termsOfUseCheckBox).click();
    cy.get(this.submitButton).should("be.visible").click();
    cy.get(this.emailAlreadyRegisteredToolTip)
      .should("be.visible")
      .should("contain", "The email entered has already")
      .and("contain", "been registered");
    cy.wait("@register").then((interception) => {
  ...
  generateRandomEmail() {
    ...
    const randomExtensionIndex = Math.floor(
      Math.random() * emailExtensions.length
    );
